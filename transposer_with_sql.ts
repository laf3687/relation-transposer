...
function setSubtypeRelationConnection(supertype: Relation, subtype: Relation, connectionType: string) {
    supertype.getPrimaryKeys().forEach(key => {
        let subTypeKeyAttribute = new Attribute(key.name, true, true, supertype, false)
...
function setOneToManyConnection(relation1: Relation, relation2: Relation, ignoreForeignKeys: boolean = false, isIdentifying: boolean = true, manyToMany: boolean = false, needsNotNull: boolean = false, connectionType: string) {
    ...
    relation1.getPrimaryKeys().forEach(key => {
        ...
        let foreignKey = new Attribute(keyName, pkBoolean, true, relation1, false) // forced (TRUE) for now
...
function buildRelations(relations: any): Map<String, Relation> {
    ...
    for (let i in relations) {
        ...
        for (let a in attributes) {
            ...
            let newAttribute = new Attribute(a, identifierBoolean, false, null, true)
...
function plantUML(noFKS: boolean = true, transposed_relations: Map<String, Relation>) {
    let connections: any[] = []
    let stringArray: string[] = []
    transposed_relations.forEach((relation) => {
        ...
        let pkNames = new Set<String>()
        relation.mustExistIn.forEach((mei) => {
            // console.log(mei.mainPKNameRecursive)
            // extra check for dually linked relations
            if (!meiSet.has(mei.relationConnection) || (meiSet.has(mei.relationConnection) && pkNames.has(mei.mainPKNameRecursive))) {
                connections.push(
                    [
                ...
                pkNames.add(mei.mainPKNameRecursive)
            }
